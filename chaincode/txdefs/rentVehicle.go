...
// Create a rent for a vehicle
var RentVehicle = tx.Transaction{
	Tag:         "rentVehicle",
	Label:       "Rent Vehicle",
	...
	Routine: func(stub *sw.StubWrapper, req map[string]interface{}) ([]byte, errors.ICCError) {
		...
		// Vehicle must be NOT rented
		rentStatus := rentMap["status"].(datatypes.RentStatus)
		if rentStatus == datatypes.Rented {
			return nil, errors.WrapError(nil, "Vehicle already rented")
		}
		...
		// Update vehicle rent
		vehicleMap["available"] = false
		vehicleMap["renter"] = renterRentKey
		vehicleMap["rent"] = rentedKey

		// Update status rent
		rentMap["status"] = datatypes.Rented
		rentMap["rentDate"] = createdAt.AsTime().Format(time.RFC3339)
		rentMap["endDate"] = nil
		rentMap["renter"] = renterRentKey
		rentMap["vehicle"] = vehicleRentKey
